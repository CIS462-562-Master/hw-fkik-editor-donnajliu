...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
    ...
    quat qf = q0;
    quat qs = q1;
    double dot = Dot(qf, qs);

    // dp negative, slerp won't take the shorter path
    if (dot < EPSILON) {
        qs = -qs;
        dot = -dot;
    }
    ...
    double omega = theta * u;
    double s0 = cos(omega) - dot * sin(omega) / sin(theta); // sin(theta - omega) / sin(theta);
    double s1 = sin(omega) / sin(theta);
    q = (s0 * qf) + (s1 * qs);
    return q.Normalize();
}
...
void quat::ToAxisAngle(vec3& axis, double& angleRad) const
{
    ...
    // when sin(angleRad/2) = 0
    float nmQ = sqrt(mQ[VW] * mQ[VW] + mQ[VX] * mQ[VX] + mQ[VY] * mQ[VY] + mQ[VZ] * mQ[VZ]);
    float normQ[4];
    normQ[0] = mQ[VW] / nmQ;
    normQ[1] = mQ[VX] / nmQ;
    normQ[2] = mQ[VY] / nmQ;
    normQ[3] = mQ[VZ] / nmQ;
    angleRad = 2.0 * acos(normQ[0]);
    if (fabs(sin(angleRad / 2.0)) > EPSILON) {
        axis[0] = normQ[1] / sin(angleRad / 2.0);
        axis[1] = normQ[2] / sin(angleRad / 2.0);
        axis[2] = normQ[3] / sin(angleRad / 2.0);
    }
}
...
void quat::FromAxisAngle(const vec3& axis, double angleRad)
{
    ...
    vec3 naxis = axis / sqrt(axis[0] * axis[0] + axis[1] * axis[1] + axis[2] + axis[2]);
    mQ[VW] = cos(angleRad / 2.0);
    mQ[VX] = sin(angleRad / 2.0) * naxis[0];
    mQ[VY] = sin(angleRad / 2.0) * naxis[1];
    mQ[VZ] = sin(angleRad / 2.0) * naxis[2];
}
